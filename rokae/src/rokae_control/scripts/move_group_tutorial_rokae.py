...
class MoveGroupTutorial(object):
  ...
  def __init__(self):
    ...
    rospy.init_node('move_group_tutorial_rokae', anonymous=True)
...
#     parts_pose=[]
#     pose_pub = rospy.Publisher('gazebo/ModelState', ModelStates, queue_size=10)

#     pose_msg = ModelStates()
#     pose_msg.model_name = 'luoshuan'

#     model_pose = rospy.wait_for_message("/gazebo/ModelState",ModelStates)
...
def get_model_pos():
    ...
    X = objstate.pose.orientation.x
    Y = objstate.pose.orientation.y
    Z = objstate.pose.orientation.z
    W = objstate.pose.orientation.w
    euler=tf.transformations.euler_from_quaternion((X,Y,Z,W)) 
    parts_pose.append([x,y,z,euler[0],euler[1],euler[2]]) 
